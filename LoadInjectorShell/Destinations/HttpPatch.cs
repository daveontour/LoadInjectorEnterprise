...
namespace LoadInjector.Destinations {
    ...
    internal class HttpPatch : IDestinationType {
        public string name = "HTTPPATCH";
        public string description = "HTTP Patch";
        ...
        public LoadInjectorGridBase GetConfigGrid(XmlNode dataModel, IView view) {
            return new HttpPatchPropertyGrid(dataModel, view);
        }

        public SenderAbstract GetDestinationSender() {
            return new DestinationHttpPatch();
        }
    }

    [RefreshProperties(RefreshProperties.All)]
    [DisplayName("HTTP Post Protocol Configuration")]
    public class HttpPatchPropertyGrid : LoadInjectorGridBase {

        public HttpPatchPropertyGrid(XmlNode dataModel, IView view) {
            _node = dataModel;
            this.view = view;
        }
    ...
    internal class DestinationHttpPatch : SenderAbstract {
        ...
        private int timeout;
        public HttpMethod httpVerb = new HttpMethod("PATCH");
                            ...
                            try {
                                ...
                                using (HttpResponseMessage response = await client.SendAsync(requestMessage)) {
                                    if (response.StatusCode == HttpStatusCode.OK || response.StatusCode == HttpStatusCode.Accepted || response.StatusCode == HttpStatusCode.Created || response.StatusCode == HttpStatusCode.NoContent) {
                                        ...
                                        logger.Warn($"Destination: {name}, HTTP Patch Response Code was not  2XX. HTTP Response Code: {response.StatusCode}. ID = {uuid}");
                                        Console.WriteLine($"Destination: {name}, HTTP Patch Response Code was not  2XX. HTTP Response Code: {response.StatusCode}. ID = {uuid}");
                                        try {
