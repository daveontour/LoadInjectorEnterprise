...
namespace LoadInjector.GridDefinitions {
    ...
    public class RateSourcePropertyGrid : LoadInjectorGridBase {

        public RateSourcePropertyGrid(XmlNode dataModel, IView view) {
            _node = dataModel;
            this.view = view;

            //if (!Parameters.SITAAMS) {
            //    Hide(new string[] { "FlightType", "FlightSetFrom", "FlightSetTo", "Sequential", "RefreshFlight" });
            //}
        }
        ...
        [RefreshProperties(RefreshProperties.All)]
        [CategoryAttribute("Source Configuration"), DisplayName("Message Per Minute"), ReadOnly(false), Browsable(true), PropertyOrder(4), DescriptionAttribute("The Message per Minute to Send")]
        public string Rate {
            get {
                double val = GetDoubleAttribute("messagesPerMinute");
                if (val < 0) {
                    val = 60;
                }
            ...
            set {
                try {
                    ...
                    MessageBox.Show($"{value} is an invalid value", "Messages Per Minute", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                SetAttribute("messagesPerMinute", value);
                view.UpdateParamBindings("XMLText");
            }
        }
        ...
        #region Message Send Profile

        [CategoryAttribute("Message Send Profile"), DisplayName("Maximum # of Messages"), ReadOnly(false), Browsable(true), PropertyOrder(1), DescriptionAttribute("The maximum number of messages to send")]
        public int MaximumNumberMessages {
            get => GetIntAttribute("maxNumMessages");
            set => SetAttribute("maxNumMessages", value);
        }

        [CategoryAttribute("Message Send Profile"), DisplayName("Maximum Run Time (sec)"), ReadOnly(false), Browsable(true), PropertyOrder(2), DescriptionAttribute("The maximum time interval for this injection line to run in seconds")]
        ...
        [RefreshProperties(RefreshProperties.All)]
        [CategoryAttribute("Message Send Profile"), DisplayName("Message Rate Profile"), Browsable(true), PropertyOrder(3), DescriptionAttribute("Maximum Number of Messages Per Minute Profile e.g '0:10,60:40,120:10'. The string is a comma seperated list of value pairs, where the values pairs are: TimeFromStartInSeconds:MaxMessagesPerMin")]
        public string MessPerMinuteProfile {
            get {
                string value = GetAttribute("maxMsgPerMinuteProfile");
                return value;
            }
            set => SetAttribute("maxMsgPerMinuteProfile", value);
        }

        [CategoryAttribute("Message Send Profile"), DisplayName("Deferred Start (sec)"), ReadOnly(false), Browsable(true), PropertyOrder(4), DescriptionAttribute("The number of seconds to defer the start of this line from the begining of execution")]
        public int DeferredStart {
            get => GetIntAttribute("deferredStart");
            set => SetAttribute("deferredStart", value);
        }

        #endregion Message Send Profile
